...
# Controller
from kinematics import foot_trajectories, solve_leg_IK
from simulation import Simulation, hills, steps, stairs, set_goal, save_terrain
from __env__ import MIN_DESIRED_TRAV, QUEUE_SIZE, GOAL_RADIUS_2, MAX_ITER_TIME, \
...
class GiadogEnv(gym.Env):
    ...
    def __init__(self, sim: Optional[Simulation]=None):
        ...
        self.sim: Optional[Simulation] = sim
        if self.sim == None:
    ...
    def __update_obs_sim(self):
        ...
        # Other data
        self.timestep         = self.sim.timestep - self.sim.initial_time
    ...
    def make_terrain(self, terrain_file: str, type: str, *args, **kwargs):
        ...
        self.goal_id = self.sim.set_goal(x, y)
    ...
    def reset(self, terrain_file: str=''):
        ...
        if self.sim == None:
            ...
            self.sim.set_goal(*self.target_dir)
            self.timestep = 0
            self.count = 0
            self.begin_time = time()
            self.__update_obs_sim()
    ...
    def traverability(self) -> float:
        ...
        trav = sum(self.trajectory) / len(self.trajectory)
        if self.is_fallen: trav *= MIN_DESIRED_TRAV - 0.1
